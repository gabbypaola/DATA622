...
mytree1 <- rpart(`Sales.Channel` ~ `Units.Sold` + `Item.Type` + `Order.Priority` + `Total.Profit`,
                data= train5k_data, 
                method='class')
mytree1
...
mytree2 <- rpart(`Sales.Channel` ~ `Units.Sold` + `Item.Type` + `Order.Priority` + `Total.Profit`,
                data= train500k_data, 
                method='class')
mytree2
...
```{r}
# Training the model
logistic_model1 <- glm(`Sales.Channel` ~ `Units.Sold` + `Item.Type` + `Order.Priority` + `Total.Profit`+`Region`, family = binomial(), train5k_data)
# Checking the model
...
```{r}
# Predictions on the training set
predictTrain1 = predict(logistic_model1, data = train5k_data, type = "response")
head(predictTrain1)
...
#Predictions on the test set
predictTest1 = predict(logistic_model1, newdata = test5k_data, type = "response")
...
```{r}
# Predictions on the training set
predictTrain = predict(logistic_model2, data = train500k_data, type = "response")
...
#Predictions on the test set
predictTest = predict(logistic_model2, newdata = test500k_data, type = "response")
