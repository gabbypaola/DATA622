...
Finally, for the random forest component of the assignment, two random forest models are created. One in which the `randomForest()` function is used without the use of hyperparameters, and another that uses the `tuneRF()` function with hyperparameters. It isn’t much of a surprise that the tuned random forest model slightly outperformed both decision tree models and the simple random forest model as noted by the metrics below.
