...
training.samples0 <- df100k_n$Total.Revenue %>%
  createDataPartition(p = 0.8, list = FALSE)
train.data  <- df100k_n[training.samples0, ]
test.data <- df100k_n[-training.samples0, ]
...
#use numerical predictors only
df100k_norm1 <- df100k_norm %>% 
  keep(is.numeric) 

#split
training.samples <- df100k_norm1$Total.Revenue %>% 
  createDataPartition(p = 0.8, list = FALSE)

train  <- df100k_norm1[training.samples, ]
test <- df100k_norm1[-training.samples, ]
...
```{r}
set.seed(12345)
#colnames(df100k_norm1)

df100k_norm2 <- df100k_norm %>% 
  select(-c("Units.Sold","Unit.Price","Unit.Cost","Total.Cost", "Total.Profit"))

#split
training.samples2 <- df100k_norm2$Total.Revenue %>% 
  createDataPartition(p = 0.8, list = FALSE)

train2  <- df100k_norm2[training.samples2, ]
test2 <- df100k_norm2[-training.samples2, ]

#train using rpart
tree2 <- rpart(Total.Revenue ~., data = train2,  method = 'anova')
...
### Predictions
```{r message=FALSE, warning=FALSE, results='hide'}
predictions2 <- predict(tree2, newdata = test2) %>% 
  bind_cols(test2)
...
data.frame(
#mean absolute error
MAE = mae(predictions2$Total.Revenue, predictions2$...1),
#rmse Root Mean Squared Error
RMSE = rmse(predictions2$Total.Revenue, predictions2$...1),
#r squared
R2 = R2(predictions2$Total.Revenue, predictions2$...1)
)
```

## Regression Decision Tree 3
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
set.seed(12345)
#colnames(df100k_norm1)

df100k_norm3 <- df100k_norm %>% 
  select(-c("Unit.Price","Unit.Cost","Total.Cost", "Total.Profit"))

#split
training.samples3 <- df100k_norm3$Total.Revenue %>% 
  createDataPartition(p = 0.8, list = FALSE)

train3  <- df100k_norm3[training.samples3, ]
test3 <- df100k_norm3[-training.samples3, ]

#train using rpart
tree3 <- rpart(Total.Revenue ~., data = train3,  method = 'anova')
