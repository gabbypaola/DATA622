...
dth <- read_tsv("https://raw.githubusercontent.com/gabbypaola/DATA622/main/Final/CountyYearRaceGenderDeaths.txt", show_col_types = FALSE)
df_dth <- dth
```
...
```{r}
df_syr2 <- df_syr %>% 
  select(c(gispropnum, month, year, month_year ,group, location, ground_syringes,
           kiosk_syringes, total_syringes, kiosk_number, kiosk_type, kiosk_site
           ))
```
...
The dataset has 14 columns and 275 rows where 9 variables are character type, and 5 variables are double type (decimals).
...
The Underlying Cause of Death dataset contains information for all counties in the state of New York for the years 2017-2021 and causes of deaths include Drug poisonings (overdose) Suicide (X60-X64), Drug poisonings (overdose) Undetermined (Y10-Y14), Drug poisonings (overdose) Unintentional (X40-X44), and All other drug-induced causes.
...
After filtering for the five bororughs the causes of death remaining are narrowed down to Drug poisonings (overdose) Suicide (X60-X64), Drug poisonings (overdose) Unintentional (X40-X44), and All other drug-induced causes. The data will be filtered further to only include Drug poisonings (overdose) Unintentional (X40-X44). 
```{r}
df_dth <- df_dth %>% 
  select(c("County","Year","Gender","Race","Deaths","Population",
...
#change column cases to lowercase  
names(df_dth) <- tolower(names(df_dth))
```
...
```{r}
b <- df %>% select(c(borough,location,typecategory,subcategory)) %>% 
  filter(borough =="Bronx")

b

length(unique(b$location))
