...
From the describe() output for the dataset, it is noted that the numeric variables have wide ranges. This will be taken care of in the preprocessing stage by scaling the variables prior to running models. The five numeric variables in the dataset are all right skewed where acres and deaths are multimodal.
...
```{r img3-with-knitr, echo=FALSE, fig.align='center', out.width='100%', fig.cap='Credit: [New Syringe Disposal Kiosks popping up at Bronx parks with highest number of used needles](https://www.ny1.com/nyc/all-boroughs/news/2018/06/08/syringe-disposal-kiosks-at-bronx-parks)'}
knitr::include_graphics("syringe_drop_box.jpg")
```
...
```{r}
num<- agg_df%>% 
  select(deaths,total_syringes,kiosk_syringes,ground_syringes)
  
cor(num,method = "pearson", use = "complete.obs") %>%
  kable(booktabs=TRUE)  %>%
  kable_styling(font_size = 9, latex_options = "scale_down")
...
Next, the data is scaled in preparation for modelling. When variables are measured at different scales, they often do not contribute equally to the analysis. For example, if the values of one variable range from 0 to 100,000 and the values of another variable range from 0 to 100, the variable with the larger range will be given a larger weight in the analysis. By scaling the variables, we can be sure that each variable contributes equally to the analysis and reduce the variance effect.
...
agg_df_norm1 <- agg_df_norm %>% 
  select(-c(month,year, month_year))
#(select(-c(month,year))

#split
training.samples <- agg_df_norm1$deaths %>% 
  createDataPartition(p = 0.8, list = FALSE)

train  <- agg_df_norm1[training.samples, ]
test <- agg_df_norm1[-training.samples, ]

train1 <- train %>% 
  select(-deaths)

bestmtry <- tuneRF(train1,train$deaths, stepFactor = 2, improve = 0.01,
...
#PREDICTIONS
trf_pred <- predict(bestmtry, newdata = test) %>% 
  bind_cols(test)

trf_pred$...1 <- as.numeric(trf_pred$...1)
```
...
#### Performance
```{r}
a <- data.frame(Model = "Tuned Random Forest: deaths",
#mean absolute error
MAE = mae(trf_pred$deaths, trf_pred$...1),
#rmse Root Mean Squared Error
RMSE = rmse(trf_pred$deaths, trf_pred$...1)
#r squared
#R2 = R2(trf_pred$deaths, trf_pred$...1)
)
a

cor(trf_pred$deaths, trf_pred$...1)^2
```

### Predicting `total_syringes`
```{r message=FALSE, warning=FALSE}
#for reproducibility
set.seed(917)
...
#split
training.samples <- agg_df_norm1$total_syringes %>% 
  createDataPartition(p = 0.8, list = FALSE)
...
train1 <- train %>% 
  select(-total_syringes)

bestmtry <- tuneRF(train1,train$total_syringes, stepFactor = 2, improve = 0.01,
                   trace=T, plot= T, doBest=TRUE, importance=TRUE)
...
#### Performance
```{r}
b <- data.frame(Model = "Tuned Random Forest: total_syringes",
#mean absolute error
MAE = mae(trf_pred$total_syringes, trf_pred$...1),
#rmse Root Mean Squared Error
RMSE = rmse(trf_pred$total_syringes, trf_pred$...1)
#r squared
#R2 = R2(trf_pred$total_syringes, trf_pred$...1)
)
...
#split
training.samples2 <- agg_df_num$deaths %>% 
  createDataPartition(p = 0.8, list = FALSE)

train_  <- agg_df_num[training.samples2, ]
test_ <- agg_df_num[-training.samples2, ]

pca_model <- caret::train(deaths ~ ., data = train_, 
                          method = 'lm', preProcess = c("center", "scale","pca"))
...
```{r}
c <- data.frame(Model = "PCA Linear Regression: deaths",
#mean absolute error
MAE = 5.607594,
#rmse Root Mean Squared Error
RMSE = 6.931028
...
#split
training.samples2 <- agg_df_num$total_syringes %>% 
  createDataPartition(p = 0.8, list = FALSE)

train_  <- agg_df_num[training.samples2, ]
test_ <- agg_df_num[-training.samples2, ]

pca_model2 <- caret::train(total_syringes ~ ., data = train_, 
                          method = 'lm', preProcess = c("center", "scale","pca"))
...
```{r}
d <- data.frame(Model = "PCA Linear Regression: total_syringes",
#mean absolute error
MAE = 4.076397,
#rmse Root Mean Squared Error
RMSE = 4.971208
...
The syringe dataset contains records of syringes collected, including how many, whether they are collected from the ground or a kiosk, and when they are collected. The Syringe Summary dataset contains records for syringes collected by both Parks staff as well as collection efforts by the [Washington Heights Corner Project](https://www.nycservice.org/organizations/index.php?org_id=1462) and [New York Harm Reduction Educators](https://nyhre.org/) nonprofit group. Each row one instance or one day's total of syringes collected in a park from the ground or safe disposal kiosk.

The parks properies dataset, provided by the Department of Parks and Recreation (DPR), identifies property managed partially or solely by NYC Parks. This data has been produced in whole or part using secondary data and is currently owned by the NYC Parks Open Data Team. Data accuracy is limited by the scale and accuracy of the original sources. Site-specific conditions should be field-verified.
...
# set.seed(212)
# 
# agg_df_norm2 <- agg_df_norm
# 
# #split
# training.samples2 <- agg_df_norm2$deaths %>% 
#   createDataPartition(p = 0.8, list = FALSE)
# 
# train_  <- agg_df_norm2[training.samples2, ]
# test_ <- agg_df_norm2[-training.samples2, ]
# 
# pcr_model <- pcr(deaths~., data = train_, validation = "CV")
# 
# summary(pcr_model)
# 
...
# 
# pcr_pred <- predict(pcr_model, test_, comps=59) %>% 
#   bind_cols(test_)
# 
# pcr_pred$...1 <- as.numeric(pcr_pred...1)
# 
# #2nd attempt
# pca_model <- caret::train(
#   total_syringes ~ .,
#   data = agg_df,
#   method = 'lm',
#   preProcess = c("pca")
...
```{r nn attempt, eval=FALSE, include=FALSE}
...
# m <- as.data.frame(model.matrix(~agg_df_norm+month_year + jurisdiction + communityboard + 
#               subcategory + typecategory + waterfront + acres + 
#               location + kiosk_type + kiosk_site + group +  race +
#               kiosk_syringes + ground_syringes + total_syringes,
#               data = agg_df_norm1))
# 
# colnames(m) <- gsub('[^[:alnum:] ]','',colnames(m))
# colnames(m) <- gsub(' ','',colnames(m))
# 
# #split
# training.samples <- m$deaths %>% 
#   createDataPartition(p = 0.8, list = FALSE)
# 
# train  <- m[training.samples, ]
# test <- m[-training.samples, ]
# 
# # colnames(m)
# 
# #build neural network
# nn <- neuralnet(totalsyringes ~ 
#                   monthyear+jurisdictionDPRDOE+communityboard202+communityboard203+communityboard204+communityboard205 +communityboard205207+communityboard206 +communityboard207+subcategoryLargePark +subcategoryNeighborhoodPark+subcategoryNeighborhoodPlgd  +subcategorySittingAreaTriangleMall+typecategoryFlagshipPark  +typecategoryJointlyOperatedPlayground+typecategoryNeighborhoodPark  +typecategoryPlayground+typecategoryRecreationFieldCourts+ typecategoryTrianglePlaza+waterfrontTRUE+acres+locationBillRaineyPark +locationClaremontPark+locationClarkPlayground +locationCrotonaPark+locationFranzSigelPark +locationNelsonPlayground+locationPattersonPlayground+locationRichmanEchoPark+locationRoseHillPark+locationRyanTriangle+locationStMarysPark +locationWalterGladwinPark+ locationWillisPlayground+kiosktypeMailbox+ kiosksiteTRUE+groupCPNYHRE+groupParks + raceWhite+kiosksyringes +groundsyringes, data = train, 
#               hidden = c(4,3),
#               linear.output = FALSE)
# 
# plot(nn,rep = "best")
...
# 
# pr.nn_ <- pr.nn$net.result * (max(m$totalsyringes) - min(m$totalsyringes)) 
#                                               + min(m$totalsyringes)
# test.r <- (test$totalsyringes) * (max(m$totalsyringes) - min(m$totalsyringes)) + 
#                                               min(m$totalsyringes)
...
#   bind_cols(test)
# 
# nn_pred$...1 <- as.numeric(nn_pred$...1)
# 
# b<- data.frame(Model = "NN",
# #mean absolute error
# MAE = mae(nn_pred$totalsyringes, nn_pred$...1),
# #rmse Root Mean Squared Error
# RMSE = rmse(nn_pred$totalsyringes, nn_pred$...1),
# #r squared
# R2 = R2(nn_pred$totalsyringes, nn_pred$...1)
# )
# 
# b
```
