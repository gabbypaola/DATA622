...
```{r}
agg_syr <- aggregate(cbind(kiosk_syringes, ground_syringes, total_syringes) ~ 
            gispropnum + borough + month + year+ month_year + group + 
            location + kiosk_site + kiosk_type,
          data = df_syr2,
          FUN = sum)
...
```{r}
df_od_dth <- df_od_dth %>% 
  mutate(month_year= as.yearmon(df_od_dth$month_year, "%b %Y"))
